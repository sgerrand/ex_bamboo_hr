defmodule BambooHR.Breaks do
  ...
  ## Parameters
    ...
    * `opts` - Optional keyword list: `:effective`, the moment to report
      availability as of. Pass a `NaiveDateTime`, a `DateTime`, a `Date`
      (taken as midnight), or a string in `YYYY-MM-DDTHH:MM:SS` form. The
      API takes no time zone, so a `DateTime` is sent as its local clock
      time with the zone dropped
  ...
  # would render a NaiveDateTime or DateTime with a space instead of the
  # `T`, and a DateTime with its offset too.
  defp format_param(%DateTime{} = value),
    do: value |> DateTime.to_naive() |> format_param()
